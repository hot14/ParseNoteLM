    ...
    def verify_token(self) -> Dict:
        ...
        try:
            ...
            if result["success"]:
                ...
                self.logger.info(
                    f"✅ 토큰 검증 성공: {user_info.get('email', 'unknown')}"
                )
            else:
...
def test_user_registration():
    ...
    for user in test_users:
        result = client.register_user(user["email"], user["username"], user["password"])
        results.append(
            {
                "email": user["email"],
    ...
    duplicate_result = client.register_user(
        test_users[0]["email"], test_users[0]["username"], test_users[0]["password"]
    )
