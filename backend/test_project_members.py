...
def test_create_project(token):
    ...
    response = requests.post(
        f"{BASE_URL}/api/projects/", json=project_data, headers=headers
    )
...
def test_get_members(project_id, token):
    ...
    response = requests.get(
        f"{BASE_URL}/api/projects/{project_id}/members", headers=headers
    )
    ...
    if response.status_code == 200:
        ...
        for member in members_data["members"]:
            print(
                f"  - {member['user_username']} ({member['user_email']}) - {member['role']}"
            )
        return members_data
    else:
...
def test_member_stats(project_id, token):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(
        f"{BASE_URL}/api/projects/{project_id}/members/stats", headers=headers
    )
...
def run_tests():
    ...
    # 4. 멤버 초대
    print("4. 멤버 초대")
    member1_id = test_invite_member(
        project_id, owner_token, "member1@test.com", "editor"
    )
    member2_id = test_invite_member(
        project_id, owner_token, "member2@test.com", "viewer"
    )
    print()
